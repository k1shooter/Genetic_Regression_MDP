...
# [수정] 복잡도 전략 인자 추가
def optimize_and_evaluate(dataset_name, X_train, y_train, X_test, y_test, target_metric, complexity_strategy,
                          seeds=None):
...
def run_mo_ga_on_dataset(dataset_name, need_seed=False):
    ...
    for target in ['f1', 'mcc']:
        for strategy in ['simple', 'weighted']:
            results_for_this_strategy = optimize_and_evaluate(dataset_name, *data, target, strategy, seeds=seeds)
            for res in results_for_this_strategy:
                res['Train_Defective_Ratio'] = train_defective_ratio
                res['Test_Defective_Ratio'] = test_defective_ratio
            dataset_results.extend(results_for_this_strategy)
...
if __name__ == "__main__":
    ...
    if all_results:
        # [수정] 출력 헤더에 Strategy와 Weighted_Cplx, 그리고 Defective Ratios 추가
        ...
        for r in all_results:
            ...
            table_data.append([
                ...
                f"{r['Test_F1']:.4f}", f"{r['Test_MCC']:.4f}", f"{r['Test_Acc']:.4f}", f"{r['Test_Defective_Ratio']:.4f}",
                fmt_form
            ])
