...
# 개별 데이터셋을 전처리하고 저장하는 함수 (SMOTE 적용)
def preprocess_and_save_data(full_url, save_directory):
    ...
    try:
        ...
        # 학습 데이터의 클래스 불균형 완화를 위한 SMOTE 적용
        # 테스트 데이터는 원본 비율을 유지해야 함
        if len(y_train) > 0:
            n_0 = (y_train == 0).sum()
            n_1 = (y_train == 1).sum()
            
            # 다수 클래스(0)가 소수 클래스(1)의 3배보다 많은 경우 비율 조정
            if n_0 > n_1 * 3:
                 print(f"   학습 데이터 비율 조정 (SMOTE 0:1=3:1): Class 1 확대 ({n_1} => {int(n_0*0.3)})")
                 smote = SMOTE(sampling_strategy=0.3, random_state=42)
                 try:
                     X_train, y_train = smote.fit_resample(X_train, y_train)
                 except Exception as e:
                     print(f"   SMOTE 적용 실패 (데이터 부족 등): {e}. 원본 데이터 유지.")
            else:
                 pass
    ...
    try: # 전처리된 데이터 저장 (RF/GP용 원본 스케일 및 DNN용 표준화 스케일)
        # RF/GP용
